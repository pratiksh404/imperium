                            ...
                            <nav v-if="navItems" class="hidden sm:flex" aria-label="Breadcrumb">
                                <ol role="list" class="flex items-center space-x-4">
                                    ...
                                    <li v-for="(navItem, index) in navItems" :key="'breadcrumb-item-' + index">
                                        <div class="flex items-center">
                                            <ChevronRightIcon class="h-5 w-5 flex-shrink-0 mr-2 text-gray-500"
                                                aria-hidden="true" />
                                            ...
                                            {{ navItem.label }}
                                            <i class="h-5 w-5 flex-shrink-0 text-gray-500" v-if="navItem.icon"
                                                :class="navItem.icon" />
                                            </Link>
                                            <div v-else>
                                                <span
                                                    class="text-sm font-medium text-gray-400 hover:text-gray-200 flex justify-start gap-1">
                                                    {{ navItem.label }}
                                                    <span class="h-5 w-5 flex-shrink-0 text-gray-500"
                                                        v-if="navItem.icon" :class="navItem.icon" />
                                                </span>
                                            </div>
                                        </div>
                                    </li>
...
const navItems = computed(() => props.nav.length > 0 ? props.nav : breadcrumbItems.value);
