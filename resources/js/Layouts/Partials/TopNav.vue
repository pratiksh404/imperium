                ...
                <template #end>
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="flex flex-col">
                            <Button id="user-menu-btn" text severity="secondary" :label="$page.props.auth.user.name"
                                class="hidden sm:flex" icon="pi pi-angle-down" iconPos="right"
                                @click="toggleUserMenu($event)" />
                            <Button class="flex sm:hidden" icon="pi pi-user" text rounded severity="secondary"
                                @click="toggleUserMenu($event)" />
                            <div id="user-menu-append" class="relative"></div>
                            <LinksMenu appendTo="#user-menu-append" :model="userMenuItems" popup ref="user-menu" :pt="{
                                root: {
