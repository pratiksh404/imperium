<template>
  <template v-if="(Object, values(items).length > 0)">
    <Popover class="relative" v-for="(flyover, flyoverIndex) in items" :key="'flyover-' + flyoverIndex">
      <PopoverButton
      ...
      <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1"
        ...
        <PopoverPanel class="absolute top-22 z-10 mt-5 flex w-screen max-w-max">
          <div
            ...
            <div class="p-4" v-if="Object.values(flyover?.items ?? {}).length > 0">
              <div v-for="item in flyover.items" :key="'flyover-item-' + flyoverIndex + '-' + item.label"
                ...
                <div
                  ...
                  <i :class="item.icon" class="size-6 p-1 text-gray-600 dark:text-white" aria-hidden="true">
                  </i>
                </div>
                <div>
                  <component :is="linkComponentResolver(item.type)" :href="item.url" v-bind="{
            ...
            <div v-if="Object.values(flyover?.footer.items ?? {}).length > 0"
              class="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50">
              <component :is="linkComponentResolver(footerItem.type)" v-for="footerItem in flyover.footer.items"
                :key="footerItem.label" :href="footerItem.url"
                class="flex items-center justify-center gap-x-2.5 p-3 font-semibold text-gray-900">
                <i :class="footerItem.icon" class="size-6 p-1 text-gray-600" aria-hidden="true">
                </i>
                {{ footerItem.label }}
              </component>
            </div>
