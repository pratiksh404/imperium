<template>
    <div class="flex justify-end" v-if="data.deleted_at === null">
        ...
        <ModuleFormOverlay name="Edit Role" :key="name + 'module-edit' + data.id" v-if="data.can.update">
            ...
            <template #form>
                <component :is="moduleForm" :name="name" method="patch" :url="route(url + '.update', data.id)"
                    :form-data="data">
                </component>
            </template>
        </ModuleFormOverlay>
        ...
        <Button size="small" @click="deleteEvent($event)" icon="pi pi-times" severity="danger" v-if="data.can.delete"
            variant="text" raised v-tooltip.top="'Delete'" />
    </div>
    <div v-if="data.deleted_at !== null">
        <Button class="mx-2" size="small" @click="restoreEvent($event)" icon="pi pi-refresh" severity="danger"
            v-if="data.can.restore" variant="text" raised v-tooltip.top="'Restore'" />
    </div>
</template>
<script setup>
...
import FieldPresentationPanel from '@/Components/Presentation/FieldPresentationPanel.vue';
...
const deleteEvent = (event) => {
    confirm.require({
        ...
        reject: () => {
            toast.add({ severity: 'info', summary: 'Info', detail: 'Data delete cancelled', life: 3000 });
        }
    });
};
...
const restoreEvent = (event) => {
    confirm.require({
        ...
        accept: () => {
            ...
            toast.add({ severity: 'success', summary: 'Success', detail: 'Data restored', life: 3000 });
        },
        reject: () => {
            toast.add({ severity: 'info', summary: 'Info', detail: 'Data restore cancelled', life: 3000 });
        }
    });
};
</script>
