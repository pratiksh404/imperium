<template v-if="field.visibility ?? true">
    <div v-if="fieldComponent" class="mt-1 my-4">
        <component :is="inputWrapper.component" v-bind="inputWrapper.props">
            ...
            <component :is="labelWrapper.component" v-bind="labelWrapper.props">
                <IconField v-if="field.iconField">
                    ...
                    <component :is="fieldComponent" v-model="resourceModel" :size="field.size"
                        :placeholder="field.floatedLabel || field.iftaLabel ? null : field.placeholder"
                        :required="field.required" :readOnly="field.readOnly" :disabled="field.disabled"
                        :class="field.class" :id="field.id" :error="error" :field="field" />
                    <InputIcon v-if="field.suffix && field.iconField" :class="field.suffix" />
                </IconField>
                <component v-else :is="fieldComponent" v-model="resourceModel" :size="field.size"
                    :placeholder="field.floatedLabel || field.iftaLabel ? null : field.placeholder"
                    :required="field.required" :readOnly="field.readOnly" :disabled="field.disabled"
                    :class="field.class" :id="field.id" :error="error" :field="field" />
                <label v-if="field.floatedLabel || field.iftaLabel" :for="field.id"><span
                        :class="`text-${error ? 'danger' : 'dark'}`">{{ field.label }}</span> <span class="text-danger"
        ...
        <Message v-if="(field.hintText ?? false) && error == null" size="small" :severity="field.hintColor"
...
const inputWrapper = computed(() => {
    return {
        component: props.field.iconField ? resolveFieldComponent('defaultInputWrapper') : resolveFieldComponent('inputGroup'),
        props: {}
    }
})
