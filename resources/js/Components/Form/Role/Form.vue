<template>
    <div>
        <form @submit.prevent="submit">
            <TextField label="Name" name="name" v-model="form.name" :error="form.errors.name" placeholder="Role Name"
...
const submit = () => {
    form[props.method](props.url, {
        ...
        onError: () => {
            toast.add({ severity: 'error', summary: 'Error', detail: 'Role not created', life: 3000 });
        }
    })
};
