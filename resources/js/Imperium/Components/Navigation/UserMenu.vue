  ...
  <div v-if="user">
    <Menu as="div" class="relative">
      <MenuButton class="-m-1.5 flex items-center p-1.5 dark:bg-black bg-white">
        ...
        <img class="size-8 rounded-full bg-gray-50 dark:bg-gray-800" :src="user.avatar" :alt="user.name" />
        <span class="hidden lg:flex lg:items-center">
          <span class="ml-4 text-sm/6 font-semibold text-gray-900 dark:text-gray-200" aria-hidden="true">{{ user.name
          }}</span>
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <MenuItems
          ...
          <component :is="linkComponentResolver(item.type)" :key="item.key" :href="item.url" :class="[
            item.active
              ? 'bg-gray-50 dark:bg-gray-800 outline-hidden'
              : '',
