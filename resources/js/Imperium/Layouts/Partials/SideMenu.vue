...
<template>
  <div
    ...
    @mouseover="collapseMouseOver"
    @mouseleave="collapseMouseLeave"
  >
    <div class="overflow-y-auto">
      <div class="hidden lg:block">
        <div
          class="flex w-full py-4 shrink-0"
          :class="collapsed ? 'px-4 justify-center' : 'justify-between px-6'"
        >
          <Link :href="route('welcome')">
            <ApplicationLogo
              class="h-10 w-auto fill-current text-surface-900 dark:text-surface-0"
            />
          </Link>
          <Button
            :icon="'pi pi-arrow-' + (initialCollapsed ? 'right' : 'left')"
            text
            @click="initialCollapseToggle"
            v-if="!collapsed"
          />
        </div>
      </div>
      <template v-if="menuGroups.length > 0">
        ...
        >
          <li>
            ...
            >
              <i
                class="pi pi-ellipsis-h p-2"
                v-tooltip="group.label"
                v-if="collapsed"
              ></i>
    ...
    <div
      v-if="user"
      class="mt-auto fixed bottom-0 bg-white dark:bg-surface-900 flex justify-between"
      :class="{ 'w-16': collapsed }"
    >
      <Link
